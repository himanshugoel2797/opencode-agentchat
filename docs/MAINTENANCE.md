...
`chat_spawn` starts persistent worker sessions in new zellij tabs — an
`@name` mention in a `chat_post` wakes an idle spawned worker by typing into
its own pane (I11). All state is
...
| `test/stress.ts` | Adversarial stress/edge suite (75 checks): multi-instance races on one worktree, lease liveness/death lifecycle, `chat_spawn` guards + deterministic worker names + non-git state-root fallback, I11 @mention-wake guard matrix (via the injected `wake` seam), corrupt-state recovery, legacy-schema backfill, trim-boundary cursor arithmetic, path-traversal refs, limits, activity-cap. ~13s. XFAIL infrastructure exists for known-bad plugin behavior (none currently). **Cannot prove spawn actually spawns or that real pane-typing wakes — see §7a.** |
...
```jsonc
{
  "ses_abc123…": {
    ...
    "persistent": true,           // chat_spawned worker: wake-eligible (I11)
    "pane": "terminal_9",         // zellij pane recorded at registration; wake types into it
    "busy": false,                // turn running (events-driven; bounds busyAt staleness)
    "busyAt": 1770000100000,
    "lastWokeAt": 0,              // last wake ping (rate limit, I11)
...
- **I11 — wakes are guarded pane-typing, never injection.** An `@name`
  mention in a `chat_post` may wake a member ONLY when ALL of: it is
  `persistent` (chat_spawn-registered under its `AGENTCHAT_NAME`; user
  sessions and transient subagents never qualify), not `busy` (busy is
  stamped by `session.status`/`session.idle` events + chat turns, with
  `BUSY_STALE_MS = 300s` so a lost idle-event can't block wakes forever),
  lease-**alive** (I5, re-probed), outside the `WAKE_COOLDOWN_MS = 60s` rate
  limit, and it has a recorded `pane`. Delivery types the envelope into the
  worker's OWN zellij pane (`zellij action write-chars -p <pane>` + Enter
  `13`) — the woken turn runs in the worker's process, visible in its tab;
  the plugin NEVER prompts another session through the client API. Mention
  matching is name-bounded (`@worker2` must not match `@worker2x`). Pane ids
  canonicalize bare-numeric `ZELLIJ_PANE_ID` → `terminal_N`. Delivery
  failures degrade to a reported line — the post still succeeds. Env
  opt-outs: `AGENTCHAT_WAKE=0` (off), `AGENTCHAT_WAKE_COOLDOWN_MS` (tests).
  Regression checks `W0`–`W10` (stress injects the PluginInput `wake` seam,
  so no real zellij is touched).
...
| `chat_agents` | — | All records, sorted; header line `name [type: X]`, then `liveness: alive / exited / (you)` (lease per I5, fail-open), `doing:` (status, last-activity, `idle`, or `-`), `last seen` (`max(lastSeen, activity.ts, statusAt)`), `rooms`. Alive workers additionally show `alive (idle)` / `alive (busy)` and spawned ones are tagged `persistent worker`. Auto-registers caller. |
...
| `chat_post` | `room`, `message` | Caller must be member. 4000-char cap. Poster auto-marks the room read at their own message. `@name` mentions of idle, alive, **persistent** members trigger an I11 pane-typing wake; result lines report each wake/skip/failure. Mentions of everyone else are inert. |
...
| G5 | zellij pane typing (I11 wake delivery) | `zellij action write-chars -p <pane> <text>` then `zellij action write -p <pane> 13` (Enter) — verified on zellij 0.44.3. Pane ids: `ZELLIJ_PANE_ID` may be bare (`9`) or named (`terminal_9`); stored canonicalized as `terminal_N`, and both forms are accepted by the write actions. Envelope must be ONE line (newlines stripped). | Live (§7a wake check): spawn a worker, `@name`-mention it from another session, confirm the pane receives the envelope and the worker posts its reply itself. Stress `W0`–`W10` cover the guards via the `wake` seam only — they CANNOT prove real typing works. |
...
npx tsx test/stress.ts    # ~13s  75 adversarial checks (races, lease liveness, spawn guards + root-fallback, wake guards, corrupt, legacy, boundaries, refs)
...
4. **Wake live check** (mandatory whenever I11/wake-delivery/G5 code changes,
   or before releasing a change that touched them): with a spawned worker from
   step 1 idle in a room, `chat_post` an `@<worker>` ping from another session.
   Confirm ALL of: the result reports `woke <worker> (typed into pane …)`; the
   envelope actually appears typed in the worker's tab; the worker replies via
   its OWN `chat_read`/`chat_post` calls (its turn ran in its process — not the
   poster's); it returns to `alive (idle)` afterwards so a second ping can wake
   it again. A repeat ping inside `WAKE_COOLDOWN_MS` must be silently
   suppressed; mentioning a transient (non-spawned) member must do nothing.
...
- **Pull-based invites.** Invitations are passive records; no mechanism may
  ...
  The ONE sanctioned exception is the I11 wake: it types a *ping* (a chat
  request, handled by the worker's own turn) into the pane of an idle worker
  the user can see and close — never a direct turn injection, never into a
  user session or transient subagent, always behind the busy/alive/cooldown
  guards.
...
- **Persistent workers are visible interactive sessions.** `chat_spawn` only
  ...
  spirit: a worker acts only when its own turn runs). Spawning is also how a
  worker acquires the only wake-capable handle (a recorded pane, I11).
...
- I11 wakes are best-effort by design: a ping that lands while the worker is busy/cooldown-gated is not queued or re-delivered (pull `chat_read` still covers it); the wake envelope is posted before room-trim bounds apply, so very long messages arrive as 300-char excerpts.
